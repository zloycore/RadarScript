...
# Конфігурація
api_id = '25293525'
api_hash = '2fc1293e908b08187d519f79bb046018'
phone_number = '+13853495776'

# Список груп для моніторингу закріплених повідомлень
pinned_monitoring_chats = [-1002844850017, -1002880304508]  # ID груп для моніторингу

# ID для заміни Premium-емодзі
EMOJI_REPLACEMENTS = {
...
async def parse_and_schedule_deletions(message):
    ...
    try:
        ...
        for line in lines:
            ...
            # Оновлений регулярний вираз для різних форматів URL
            match = re.search(
                r'(?:https?://)?t\.me/(?:c/)?(\w+)/(\d+)',
                url,
                re.IGNORECASE
            ...
            # Отримуємо повний об'єкт каналу
            try:
...
async def unpin_message_with_retry(chat_id, message_id, retries=3, default_delay=882):
    # Додано перевірку на message_id == 7733
    if message_id == 7733:
...
async def monitor_pinned_messages():
    """Моніторинг закріплених повідомлень у групах"""
    while True:
        try:
            for chat_id in pinned_monitoring_chats:
                try:
                    ...
                    async for msg in client.iter_messages(chat_id, limit=100):
                        # Додано перевірку на msg.id == 7733
...
async def unpin_existing_messages():
    """Відкріплення всіх існуючих закріплених повідомлень у групах"""
    for chat_id in pinned_monitoring_chats:
        try:
            chat = await client.get_entity(chat_id)
            if not hasattr(chat, 'megagroup') or not chat.megagroup:
            ...
            offset_id = 0
            limit = 5  # Обробка по 1000 повідомлень за раз
            total_unpinned = 0
            while True:
                pinned_messages = []
                ...
                for msg in pinned_messages:
                    try:
                        if await unpin_message_with_retry(chat_id, msg.id):
                            ...
                            total_unpinned += 1
                        await asyncio.sleep(10)  # Затримка 5 секунд для уникнення rate limits
                    except Exception as e:
                        print(f"❌ Помилка відкріплення повідомлення {msg.id}: {e}")
                offset_id = pinned_messages[-1].id
                await asyncio.sleep(10)  # Затримка між ітераціями
...
async def main():
    ...
    # Очищаємо базу даних для групи -1002880304508
    clear_pinned_messages_db(-1002880304508)
    
    # Підключаємося до Telegram спочатку
    await client.start(phone_number)
    print("🟢 Скрипт запущено")
    
    # Перевіряємо, чи це дійсно групи
    for chat_id in pinned_monitoring_chats:
        try:
            chat = await client.get_entity(chat_id)
            if not hasattr(chat, 'megagroup') or not chat.megagroup:
    ...
    # Завантажуємо заплановані видалення
    ...
    # Відкріплюємо існуючі повідомлення
